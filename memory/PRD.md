...
## Fitur (2026-06) — Iterasi 10 (restore backup + laporan rinci + edit tanggal)
- Restore backup: POST /api/backup/import (owner, multipart 'file') memulihkan seluruh data dari file backup Excel aplikasi (12 sheet). Idempotent: docs bertanda `_import:True` dihapus lalu diinsert ulang; master (part/jasa/pelanggan) di-upsert by code/phone/plate; data yang diinput manual aman. Join item↔transaksi via transaction_id (uuid) yang sama; metadata transaksi diambil dari sheet Transaksi via kedekatan created_at (<600s) atau kolom id (backup versi baru). Counter trx/invoice/customer/sale di-bump agar penomoran lanjut. Diuji dengan backup user: 601 part aktif, 400 jasa, 23 pelanggan, 36 motor, 38 transaksi, 35 pembayaran (omzet Rp 5.070.000 cocok).
- Backup export ditingkatkan agar round-trip: sheet Transaksi kini punya kolom `id` + `next_recommendation`; Item Transaksi punya `cost`; Belanja punya supplier/discount/het/ongkir/unit_price/payment_method; ada sheet baru "Penjualan Langsung".
- Laporan Penjualan Servis (GET /api/reports/service-sales, layar /laporan-penjualan tab Servis): per nota — tanggal, pelanggan, motor, mekanik, saran servis, jasa, suku cadang (harga jual/beli/laba per item), total jasa/part/laba. Cost part di-fallback lookup by code untuk data restore. Export Excel.
- Laporan Jualan Langsung (GET /api/reports/direct-sales, tab Jualan): rincian item (jual/beli/diskon/laba), pelanggan, metode, total, laba. Export Excel.
- Laporan Belanja Rinci (GET /api/reports/purchases, layar /laporan-belanja): Pembelian Part (nama, supplier, qty, harga satuan, HET, diskon, ongkir, total, metode), Kasbon Mekanik, Kasbon Owner, Biaya Operasional; ringkasan per metode CASH/HUTANG/TRANSFER. Export Excel.
- Belanja diperluas: ExpenseIn + form menambah supplier, unit_price, HET, ongkir, discount, payment_method (CASH/HUTANG/TRANSFER); kategori BENGKEL baru "Kasbon Mekanik"/"Gaji Mekanik". PUT /api/expenses/{eid} untuk edit termasuk tanggal.
- Edit tanggal transaksi (owner): PUT /api/transactions/{tid}/date & PUT /api/sales/{sid}/date — sinkron paid_at/date pembayaran & service_history agar omzet/laporan ikut tanggal baru. Tombol "Ubah Tanggal" + sheet di layar detail transaksi.
- FIX: build_detail tahan terhadap transaksi hasil restore tanpa customer_id/vehicle_id.
- Testing agent iterasi 2: 14/14 backend + semua flow frontend PASS.
