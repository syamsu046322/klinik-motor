...
export default function TransaksiDetail() {
  ...
  const sopItems = useQuery({ queryKey: ["sop-items"], queryFn: () => api<any[]>("/service-checklist-items") });
  useEffect(() => {
    if (!t) return;
    const done: Record<string, boolean> = {};
    (t.sop_checklist ?? []).forEach((c: any) => { done[c.name] = !!c.ok; });
    setSopState((prev) => (Object.keys(prev).length ? prev : done));
  }, [t?.id]);
    ...
    <View style={styles.root} testID="transaction-detail-screen">
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 160 }} refreshControl={<RefreshControl refreshing={q.isRefetching} onRefresh={q.refetch} tintColor={colors.brandPrimary} />}>
        ...
        <Card testID="totals-card">
          ...
          {user?.role === "owner" ? (
            <View style={{ marginTop: 8, borderTopWidth: 1, borderTopColor: colors.divider, paddingTop: 8 }} testID="laba-nota-box">
              <KV k="Modal Part (HPP)" v={rupiah(totals.modal_part)} mono />
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 2 }}>
                <Text style={{ fontSize: 14, fontWeight: "700", color: colors.success }}>LABA PER NOTA</Text>
                <Mono style={{ fontSize: 16, fontWeight: "700", color: colors.success }} testID="laba-nota-value">{rupiah(totals.laba)}</Mono>
              </View>
            </View>
          ) : null}
        </Card>
        ...
        {!isCancelled && !isPaid && canMech && (status === "DIPROSES" || status === "MENUNGGU_PERSETUJUAN") && (sopItems.data?.length ?? 0) > 0 ? (
          <Card testID="sop-card">
            <SectionTitle title="CHECKLIST SOP (WAJIB SEBELUM SELESAI)" />
            <Text style={{ color: colors.muted, fontSize: 12, marginBottom: 8 }}>Semua item wajib OK sebelum servis bisa diselesaikan & nota dicetak.</Text>
            {sopItems.data!.map((it: any) => {
              const ok = !!sopState[it.name];
              return (
                <Pressable key={it.id} onPress={() => setSopState((s) => ({ ...s, [it.name]: !s[it.name] }))}
                  style={{ flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.divider }} testID={`sop-item-${it.name}`}>
                  <Ionicons name={ok ? "checkbox" : "square-outline"} size={24} color={ok ? colors.success : colors.muted} />
                  <Text style={{ flex: 1, fontSize: 15, color: colors.onSurface }}>{it.name}</Text>
                  <Badge label={ok ? "OK" : "BELUM"} tone={ok ? "success" : "warning"} />
                </Pressable>
              );
            })}
            <Button title="Simpan Checklist" variant="dark" small style={{ marginTop: 10 }} loading={mut.isPending} testID="sop-save-button"
              onPress={() => mut.mutate({ path: "/sop", body: { items: (sopItems.data ?? []).map((it: any) => ({ name: it.name, ok: !!sopState[it.name] })) } })} />
          </Card>
        ) : null}

        {!isCancelled && (can("mekanik", "kasir")) ? <Button title="Batalkan Transaksi (Password Owner)" variant="ghost" onPress={() => { setCancelPw(""); setCancelReason(""); setCancelOpen(true); }} testID="cancel-transaction-button" style={{ borderWidth: 2, borderColor: colors.error }} /> : null}
      </ScrollView>
      ...
      <Sheet visible={cancelOpen} onClose={() => setCancelOpen(false)} title="Batalkan Transaksi" testID="cancel-sheet" scroll={false}>
        ...
        <Input label="Password Owner *" value={cancelPw} onChangeText={setCancelPw} secureTextEntry testID="cancel-owner-password-input" />
        <Text style={styles.paraMuted}>Semua pembatalan wajib konfirmasi password Owner.{isPaid ? " Faktur sudah dibayar: stok part akan dikembalikan dan pembatalan masuk laporan Owner." : ""}</Text>
        <View style={{ height: 8 }} />
        <Button title="Ya, Batalkan" variant="danger" loading={mut.isPending} disabled={!cancelReason.trim() || !cancelPw} testID="cancel-submit-button"
          onPress={() => mut.mutate({ path: "/cancel", body: { reason: cancelReason, owner_password: cancelPw } }, { onSuccess: () => { setCancelOpen(false); setCancelPw(""); } })} />
      </Sheet>
