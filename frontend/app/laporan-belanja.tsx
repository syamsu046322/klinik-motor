...
import { Button, Empty, Header, Loading, Mono, SectionTitle, useToast } from "@/src/components/ui";
...
export default function LaporanBelanja() {
  ...
  const rep = useQuery({ queryKey: ["report-purchases", month], queryFn: () => api<any>(`/reports/purchases${qs({ month })}`) });
  const d = rep.data;

  const doExport = async () => {
    setBusy(true);
    try { toast.show(await exportReportXlsx("purchases", month), "success"); }
  ...
  const MonthChips = (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
      {[0, 1, 2, 3, 4, 5].map((o) => {
        const m = monthKey(o);
        return (
          <Text key={m} onPress={() => setMonth(m)} style={[styles.chip, month === m && styles.chipSel]} testID={`purchase-month-${m}`}>{monthLabel(m)}</Text>
        );
      })}
    </ScrollView>
  );
        ...
        {rep.isLoading || !d ? <Loading /> : (
          <>
            <View style={styles.totalBox}>
              <Text style={styles.totalLabel}>TOTAL BELANJA · {monthLabel(month)}</Text>
