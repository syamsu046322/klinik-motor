...
import { Badge, Button, Chips, Confirm, Empty, Input, Loading, Mono, Sheet, useToast } from "@/src/components/ui";
...
export default function Stok() {
  ...
  const cleanMut = useMutation({
    mutationFn: () => api<{ moved_to_trash: number; duplicates_merged: number }>("/parts/cleanup", { method: "POST" }),
    onSuccess: (r) => { qc.invalidateQueries({ queryKey: ["parts"] }); qc.invalidateQueries({ queryKey: ["import-trash"] }); qc.invalidateQueries({ queryKey: ["dashboard"] }); setCleanOpen(false); toast.show(`${r.moved_to_trash} part sampah dipindah, ${r.duplicates_merged} duplikat digabung`, "success"); },
    onError: (e: any) => toast.show(e.message, "error"),
  });
  ...
  return (
    <View style={styles.root} testID="stock-screen">
        ...
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          {user?.role === "owner" ? (
            <View style={{ flexDirection: "row", gap: 16 }}>
              <Pressable onPress={() => setCleanOpen(true)} hitSlop={8} testID="stock-cleanup-button">
                <Ionicons name="sparkles-outline" size={22} color={colors.onSurfaceInverse} />
              </Pressable>
      ...
      <Confirm visible={cleanOpen} title="Bersihkan Part Sampah" danger confirmLabel="YA, BERSIHKAN" loading={cleanMut.isPending}
        message="Part sampah (kode Z/L, nama mengandung 'Kosong'/'Low') akan dipindah ke Sampah Import, dan kode duplikat akan digabung. Data bisa dipulihkan dari menu Sampah Import."
        onCancel={() => setCleanOpen(false)} onConfirm={() => cleanMut.mutate()} testID="stock-cleanup-confirm" />
    </View>
  );
}
